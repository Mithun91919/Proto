...
function OptionHead({ code, name, tradeoff }: { code: string; name: string; tradeoff: string }) {
  return (
...
export default function SummaryOptionsPage() {
  return (
    <div className="ds-scope pb-32">
      ...
      {/* ══ Round three ═══════════════════════════════════════════════ */}
      <div className="mx-auto w-full max-w-[85rem] px-5 pt-28 md:px-8 md:pt-36">
        <p className="eyebrow">Round three</p>
        <h2 className="display-title mt-4 max-w-[26ch]" style={{ fontSize: "2rem", lineHeight: 1.1 }}>
          Four other ways in.
        </h2>
        <p className="mt-5 max-w-[62ch] text-lg leading-8" style={{ color: "var(--ink-soft)" }}>
          D and F both summarise by diagramming. These try four different jobs instead: assert, show the
          product, lay out the argument, and contrast the two systems.
        </p>
      </div>
      ...
      <div className="mx-auto w-full max-w-[85rem] px-5 pb-6 pt-16 md:px-8">
        <OptionHead
          code="G"
          name="The claim, at poster scale"
          tradeoff="No diagram at all — typography carries it, which is the site's first stated principle. Most confident, and the only one that needs no maintenance when the product changes."
        />
      </div>
      <div className="ds-pull">
        <div className="ds-pull-inner" style={{ color: "var(--ds-dark-ink)" }}>
          <span className="ds-pull-dots" aria-hidden />
          <p
            className="display-title max-w-[17ch]"
            style={{ fontSize: "clamp(2.2rem, 5.4vw, 4.2rem)", lineHeight: 1.02, fontWeight: 400 }}
          >
            Most support apps are good at{" "}
            <span style={{ color: "var(--ds-dark-muted)" }}>collecting tickets</span>. This one had to{" "}
            <span style={{ color: "var(--ds-mint)" }}>prevent them</span>.
          </p>
          <p className="mt-9 max-w-[52ch] text-base leading-7" style={{ color: "var(--ds-dark-muted)" }}>
            Frontline support for Walmart store associates — facilities and technology, on a phone, across a
            ~580K-device footprint.
          </p>
        </div>
      </div>

      {/* ── H · product-anchored ──────────────────────────────────────── */}
      <div className="mx-auto w-full max-w-[85rem] px-5 pt-24 md:px-8">
        <OptionHead
          code="H"
          name="Three moves, on the product"
          tradeoff="Ties the summary to the thing itself, so it proves rather than describes. Costs the most to keep true — the callouts have to track the screens."
        />
        <div className="mt-9 grid items-center gap-10 lg:grid-cols-[1.25fr_0.75fr] lg:gap-16">
          <div className="ds-env-dark rounded-2xl p-6" style={{ background: "var(--ds-dark)" }}>
            <Image
              src="/work/store-support/hero-composite.png"
              width={2400}
              height={2188}
              alt="Three FixIt screens: home, submit an issue, and refrigeration alarms."
          ...
          <div className="flex flex-col">
            {[
              { n: "01", t: "One front door", d: "Facilities and technology in the same app — refrigeration alarms beside technology tickets." },
              { n: "02", t: "The fix comes first", d: "Step one of two is the resolution. The ticket is step two, and only if the fix fails." },
              { n: "03", t: "The loop stays open", d: "Track the work order, add photos, escalate or recall when a repair does not hold." },
            ].map((row, i) => (
              <div key={row.n} className={`py-6 ${i > 0 ? "ds-rule" : ""}`}>
                <CompactNumeral value={row.n} />
                <p className="display-title mt-4" style={{ fontSize: "1.3rem" }}>
                  {row.t}
                </p>
                <p className="body-sm mt-2 max-w-[38ch]" style={{ color: "var(--ink-soft)" }}>
                  {row.d}
                </p>
              </div>
            ))}
      ...
      {/* ── I · the argument as an index ──────────────────────────────── */}
      <div className="mx-auto w-full max-w-[85rem] px-5 pt-28 md:px-8">
        <OptionHead
          code="I"
          name="The argument, numbered"
          tradeoff="S6 rows doing double duty — a summary you can also navigate by. The only option that earns its height by being useful twice."
        />
        <div className="mt-6">
          <EditorialNumberedRows
            rows={[
              {
                number: "01",
                title: "Anything that breaks, one way in",
                detail:
                  "Five categories in the associate's own language, an asset-tag scanner for equipment, and search that carries the query forward instead of dead-ending.",
              },
              {
                number: "02",
                title: "The fix before the form",
                detail:
                  "Guided resolution runs as step one of two — written for software faults, photographic for physical ones. A ticket only exists if the fix did not hold.",
              },
              {
                number: "03",
                title: "A loop the associate keeps hold of",
                detail:
                  "Work orders carry trade, equipment and status; alarms raise them automatically. Photos, notes, escalation and rating all stay with the original record.",
              },
            ]}
          />
        </div>
      ...
      {/* ── J · system contrast ───────────────────────────────────────── */}
      <div className="mx-auto w-full max-w-[85rem] px-5 pb-24 pt-28 md:px-8">
        <OptionHead
          code="J"
          name="Two systems, compared"
          tradeoff="E2 at full width. Compares the shape of the old and new models rather than the journey — closest to how a design lead would frame it, least legible to a recruiter."
        />
        <div className="mt-9">
          <BeforeAfterModel
            before={{
              heading: "Built to collect",
              body: "A flat category list written around the support desk, a form at the end of every path, and troubleshooting left to the person receiving the ticket.",
              lineWidths: [100, 88, 76, 64, 52],
            }}
            after={{
              heading: "Built to resolve",
              body: "Categories in the associate's language, resolution ahead of the form, and escalation that arrives with the failed attempt already attached.",
              lineWidths: [100, 64],
            }}
          />
        </div>
